...
# lib = cdll.LoadLibrary('../bin/UTaggerR64.dll') # dll 로드 윈도우
...
def Init_Utagger():
    ...
    # lib = cdll.LoadLibrary('UTaggerR64.dll') # dll 로드 윈도우
    lib = cdll.LoadLibrary('modules/utagger/bin/UTaggerR64.dll') # dll 로드 윈도우
    
    #설정파일의 위치를 절대경로로 구하기. py에서의 현재경로와, dll에서의 현재경로가 다르기 때문.
    hlx_pass = os.path.join( os.path.dirname(sys.argv[0]), "C:/Users/User/source/repos/JeanMandoo/modules/utagger/Hlxcfg.txt")
    ...
    os.chdir("C:/Users/User/source/repos/JeanMandoo/modules/utagger/bin")
    # **** 여기 학습 파일 로딩 안 됨!!!
